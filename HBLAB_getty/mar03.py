...
def get_jump_fps(fps, frame_rate):
    ...
    if fps == 1:
        return frame_rate
    if fps == 3:
        if frame_rate == 25 or frame_rate == 24:
            return 10
        if frame_rate == 30:
            return 8
        if frame_rate == 60:
...
def extract_vid(vidcap, frame_rate, start_frame, end_frame, fps_str, path_out, filename, txt_log):
    ...
    frame_extract = start_frame
    fps = get_fps(fps_str)
    jump_fps = get_jump_fps(fps, frame_rate)
    ...
    while vidcap.isOpened():
        ...
        if count_jum < fps:
            if curent_frame == int(frame_extract) and frame_extract < end_frame:
                filename_new = path_out + '\\' + filename + "_{:05d}.jpg".format(curent_frame+1)
                ...
                if count_jum >= fps:
                    print("--------------")
                    # print(curent_frame)
                    print("time: {}".format(int(curent_frame/frame_rate)+1))
                    frame_extract = (int(curent_frame/frame_rate)+1)*frame_rate
                    count_jum = 0
                    # curent_frame = frame_extract - 1
                else:
                    frame_extract += jump_fps
            curent_frame += 1
...
if __name__=="__main__":
    # input_loc = r'\\server10\Hblab\1.Customer\1.Video\part_01_50video'
    # output_loc = r'\\server10\Hblab\3.Working\1.Task\20200303'
    input_loc = r"E:\video"
    output_loc = r"E:\video"
